...
//    Connect to the  database 
  $mysqli = new mysqli(DATABASE_HOST, DATABASE_USER, DATABASE_PASS, DATABASE_NAME);
    ...
    $query = "SELECT * 
              FROM invoice i
              JOIN `order` o
              ON   o.invoice=i.invoice
              WHERE i.invoice = o.invoice
              ORDER BY i.invoice";
    ...
    if($results)
    {
        print '  <div class="card-body--"> <div class="table-stats order-table ov-h"><table class="table" ><thead><tr>
        ...
        while($row=$results->fetch_assoc())
        {
            print '
                 <tr>
                   ...
                   if($row['status'] == "open")
                   {
                       print '<span class="badge badge-pending">Open</span></a> &nbsp';
                   }
                   elseif($row['status'] == "paid")
                   {
                    print '<span class="badge badge-complete">Paid</span></a> &nbsp';   
                   }
...
{
//    connect to the database 
       $mysqli = new mysqli(DATABASE_HOST,DATABASE_USERNAME,DATABASE_PASS,DATABASE_NAME);
       // output any connection error
	if ($mysqli->connect_error) {
	    die('Error : ('. $mysqli->connect_errno .') '. $mysqli->connect_error);
	}
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '
			    <tr>
				    ...
				    <td>$'.$row["price"].'</td>
				    <td><a href="product.php?id='.$row["product_id"].'" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a> <a data-product-id="'.$row['id'].'" class="btn btn-danger btn-xs delete-product"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>
			    </tr>
		    ';
		}
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '
			    <tr>
				    ...
				    <td>'.$row["email"].'</td>
				    <td>'.$row["phone"].'</td>
				    <td><a href="users.php?id='.$row["id"].'" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a> <a data-customer-id="'.$row['id'].'" class="btn btn-danger btn-xs delete-customer"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>
			    </tr>
		    ';
		}
